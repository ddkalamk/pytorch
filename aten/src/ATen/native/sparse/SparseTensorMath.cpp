...
#if 0
template <typename T>
inline void add_dense_sparse_axpy(int64_t aligned64_size, int32_t left_size, const T* x, T* y, T cast_value)
{
  AT_ASSERT(aligned64_size % 64 == 0);
  int64_t vb = 0;
  for( ; vb < aligned64_size; vb += 64) {
    #pragma unroll(8)
    for (int i = 0 ; i < 8; i ++) {
      PRAGMA_SIMD
      for (int j = 0; j < 8; j ++ ) {
        y[i * 8 + j] += x[i * 8 + j] * cast_value;
      }
    }
  }
  if (left_size > 0) {
    for(auto l = 0; l < left_size; l++) {
      y[aligned64_size + l] += cast_value * x[aligned64_size + l];
    }
  }
}
#endif
...
inline void add_dense_sparse_worker_critical_section_cpu(Tensor& r, Scalar value,
  ...
  auto aligned64_size = ((values_dense_size >> 5) << 5);
  auto left_size = values_dense_size - aligned64_size;
  at::parallel_for(0, sparse_nnz, 64, [&](int64_t start, int64_t end) {
    for (auto k = start; k < end; k++) {
      ...
      int64_t vb = 0;
      for( ; vb < aligned64_size; vb += 32) {
        #pragma unroll(4)
        for (int i = 0 ; i < 4; i ++) {
          PRAGMA_SIMD
          for (int j = 0; j < 8; j ++ ) {
            r_index[i * 4 + j] += v_index[i * 4 + j] * cast_value;
          }
        }
      }
      if (left_size > 0) {
        for(auto l = vb; l < values_dense_size; l++) {
          r_index[l] += cast_value * v_index[l];
        }
      }
      //add_dense_sparse_axpy<scalar_t>(aligned64_size, left_size, v_index, r_index, cast_value);
...
Tensor& add_out_dense_sparse_cpu(Tensor& r, const Tensor& dense, const SparseTensor & sparse_, Scalar value) {
  ...
  bool result_is_contiguous =resultBuffer.is_contiguous();
